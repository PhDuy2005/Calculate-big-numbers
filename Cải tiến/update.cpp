...
///<summary>
///This function calculates the multiply of 2 numbers
///</summary>
/// <return>An array contained the result of X*Y</return>
void multiBigNumber(int X[], int Y[], int& len, int result[]) {
	int tempMul[50][100];
	int carry = 0;
	char temp;
	int current_digit;
	...
	for (int Yi = 0; Yi < len; Yi++) { //Data preparation
		for (int Xj = 0; Xj < max_len; Xj++) {
			if (Xj < Yi) tempMul[Yi][Xj] = 0;
			else if (Xj > Yi + len) tempMul[Yi][Xj] = 0;
		}
	}

	for (int Yi = 0; Yi < len; Yi++) { //Take X multiply by each digit of Y
		current_digit = Y[Yi];
		carry = 0;
		for (int Xj = 0; Xj < len; Xj++) {
			temp = current_digit * X[Xj] + carry;
			carry = temp / 10;
			tempMul[Yi][Xj + Yi] = temp % 10;
		}
	}
